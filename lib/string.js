...
Util.inherits(internals.String, Any);
...
internals.compare = function (type, compare) {
    ...
    return function (limit, encoding) {
        ...
        Patch.assert((Patch.isInteger(limit) && limit >= 0) || isRef, 'limit must be a positive integer or reference');
        Patch.assert(!encoding || Buffer.isEncoding(encoding), 'Invalid encoding:', encoding);
...
internals.String.prototype.regex = function (pattern, name) {
    ...
    Patch.assert(pattern instanceof RegExp, 'pattern must be a RegExp');
...
internals.String.prototype.email = function (isEmailOptions) {
    ...
    if (isEmailOptions) {
        Patch.assert(typeof isEmailOptions === 'object', 'email options must be an object');
        Patch.assert(typeof isEmailOptions.checkDNS === 'undefined', 'checkDNS option is not supported');
        Patch.assert(typeof isEmailOptions.tldWhitelist === 'undefined' ||
            typeof isEmailOptions.tldWhitelist === 'object', 'tldWhitelist must be an array or object');
        Patch.assert(typeof isEmailOptions.minDomainAtoms === 'undefined' ||
            _.isInteger(isEmailOptions.minDomainAtoms) && isEmailOptions.minDomainAtoms > 0,
            'minDomainAtoms must be a positive integer');
        Patch.assert(typeof isEmailOptions.errorLevel === 'undefined' || typeof isEmailOptions.errorLevel === 'boolean' ||
            (_.isInteger(isEmailOptions.errorLevel) && isEmailOptions.errorLevel >= 0),
...
internals.String.prototype.ip = function (ipOptions) {
    ...
    ipOptions = ipOptions || {};
    Patch.assert(typeof ipOptions === 'object', 'options must be an object');

    if (ipOptions.cidr) {
        Patch.assert(typeof ipOptions.cidr === 'string', 'cidr must be a string');
        ipOptions.cidr = ipOptions.cidr.toLowerCase();

        Patch.assert(ipOptions.cidr in Ip.cidrs, 'cidr must be one of ' + Object.keys(Ip.cidrs).join(', '));
    ...
    if (ipOptions.version) {
        ...
        Patch.assert(ipOptions.version.length >= 1, 'version must have at least 1 version specified');

        versions = [];
        for (let i = 0; i < ipOptions.version.length; ++i) {
            let version = ipOptions.version[i];
            Patch.assert(typeof version === 'string', 'version at position ' + i + ' must be a string');
            version = version.toLowerCase();
            Patch.assert(Ip.versions[version], 'version at position ' + i + ' must be one of ' + Object.keys(Ip.versions).join(', '));
...
internals.String.prototype.uri = function (uriOptions) {
    ...
    if (uriOptions) {
        Patch.assert(typeof uriOptions === 'object', 'options must be an object');

        if (uriOptions.scheme) {
            Patch.assert(uriOptions.scheme instanceof RegExp || typeof uriOptions.scheme === 'string' || Array.isArray(uriOptions.scheme), 'scheme must be a RegExp, String, or Array');

            if (!Array.isArray(uriOptions.scheme)) {
                uriOptions.scheme = [uriOptions.scheme];
            }

            Patch.assert(uriOptions.scheme.length >= 1, 'scheme must have at least 1 scheme specified');

            // Flatten the array into a string to be used to match the schemes.
            for (let i = 0; i < uriOptions.scheme.length; ++i) {
                const scheme = uriOptions.scheme[i];
                Patch.assert(scheme instanceof RegExp || typeof scheme === 'string', 'scheme at position ' + i + ' must be a RegExp or String');
                ...
                if (scheme instanceof RegExp) {
                    ...
                    Patch.assert(/[a-zA-Z][a-zA-Z0-9+-\.]*/.test(scheme), 'scheme at position ' + i + ' must be a valid scheme');
                    customScheme = customScheme + _.escapeRegExp(scheme);
                }
            }
        }
...
internals.String.prototype.replace = function (pattern, replacement) {
    ...
    Patch.assert(pattern instanceof RegExp, 'pattern must be a RegExp');
    Patch.assert(typeof replacement === 'string', 'replacement must be a String');
