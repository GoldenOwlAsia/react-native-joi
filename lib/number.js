...
Util.inherits(internals.Number, Any);
...
internals.compare = function (type, compare) {
    ...
    return function (limit) {
        ...
        Patch.assert(isNumber || isRef, 'limit must be a number or reference');
...
internals.Number.prototype.multiple = function (base) {
    ...
    Patch.assert(_.isInteger(base), 'multiple must be an integer');
    Patch.assert(base > 0, 'multiple must be greater than 0');
...
internals.Number.prototype.integer = function () {
    ...
    return this._test('integer', undefined, (value, state, options) => {
        ...
        return _.isInteger(value) ? null : this.createError('number.integer', { value }, state, options);
    });
};
...
internals.Number.prototype.precision = function (limit) {
    ...
    Patch.assert(_.isInteger(limit), 'limit must be an integer');
    Patch.assert(!('precision' in this._flags), 'precision already set');
