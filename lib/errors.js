...
internals.Err.prototype.toString = function () {
    ...
    let format = Patch.reach(localized, this.type) || Patch.reach(Language.errors, this.type);
    ...
    if (!hasKey && !skipKey) {
        format = (Patch.reach(localized, 'key') || Patch.reach(Language.errors, 'key')) + format;
    }

    let wrapArrays = Patch.reach(localized, 'messages.wrapArrays');
    ...
    const message = format.replace(/\{\{(\!?)([^}]+)\}\}/g, ($0, isSecure, name) => {
        ...
        const value = Patch.reach(this.context, name);
        const normalized = internals.stringify(value, wrapArrays);
        return (isSecure ? Escape.escapeHtml(normalized) : normalized);
    });
...
internals.getPath = function (item) {
    ...
    const recursePath = (it) => {
        ...
        const reachedItem = Patch.reach(it, 'context.reason.0');
...
internals.annotate = function () {
    ...
    const obj = Patch.clone(this._object || {});
