...
Util.inherits(internals.Object, Any);
...
internals.Object.prototype._base = function (value, state, options) {
    ...
    if (target === value) {
        if (type === 'object') {
            ...
            target.prototype = Patch.clone(value.prototype);
        }
    ...
    const childOptions = options.language && options.language.label ?
        Patch.applyToDefaults(options, { language: { label: null } }, true) :
        options;
...
internals.Object.prototype.keys = function (schema) {
    ...
    Patch.assert(schema === null || schema === undefined || typeof schema === 'object', 'Object schema must be a valid object');
    Patch.assert(!schema || !schema.isJoi, 'Object schema cannot be a joi schema');
...
internals.Object.prototype.arity = function (n) {
    ...
    Patch.assert(_.isInteger(n) && n >= 0, 'n must be a positive integer');
...
internals.Object.prototype.minArity = function (n) {
    ...
    Patch.assert(_.isInteger(n) && n > 0, 'n must be a strict positive integer');
...
internals.Object.prototype.pattern = function (pattern, schema) {
    ...
    Patch.assert(pattern instanceof RegExp, 'Invalid regular expression');
    Patch.assert(schema !== undefined, 'Invalid rule');
...
internals.Object.prototype.xor = function () {
    ...
    const peers = _.flatten(Array.prototype.slice.call(arguments));
...
internals.Object.prototype.requiredKeys = function (children) {
    ...
    children = _.flatten(Array.prototype.slice.call(arguments));
...
internals.Object.prototype.rename = function (from, to, options) {
    ...
    Patch.assert(typeof from === 'string', 'Rename missing the from argument');
    Patch.assert(typeof to === 'string', 'Rename missing the to argument');
    Patch.assert(to !== from, 'Cannot rename key to same name:', from);

    for (let i = 0; i < this._inner.renames.length; ++i) {
        Patch.assert(this._inner.renames[i].from !== from, 'Cannot rename the same key multiple times');
    }
    ...
    obj._inner.renames.push({
        from,
        to,
        options: Patch.applyToDefaults(internals.renameDefaults, options || {})
    });
...
internals.groupChildren = function (children) {
    ...
    for (let i = 0; i < children.length; ++i) {
        ...
        Patch.assert(typeof child === 'string', 'children must be strings');
...
internals.Object.prototype.applyFunctionToChildren = function (children, fn, args, root) {
    ...
    Patch.assert(remaining.length === 0, 'unknown key(s)', remaining.join(', '));
...
internals.Object.prototype._dependency = function (type, key, peers) {
    ...
    for (let i = 0; i < peers.length; ++i) {
        Patch.assert(typeof peers[i] === 'string', type, 'peers must be a string or array of strings');
    }
...
internals.nand = function (value, peers, parent, state, options) {
    ...
    const values = Patch.clone(peers);
...
internals.Object.prototype.describe = function (shallow) {
    ...
    if (description.rules) {
        for (let i = 0; i < description.rules.length; ++i) {
            ...
            // Coverage off for future-proof descriptions, only object().Patch.assert() is use right now
    ...
    if (this._inner.dependencies.length) {
        description.dependencies = Patch.clone(this._inner.dependencies);
    }
...
internals.Object.prototype.assert = function (ref, schema, message) {
    ...
    ref = Cast.ref(ref);
    Patch.assert(ref.isContext || ref.depth > 1, 'Cannot use assertions for root level references - use direct key rules instead');
...
internals.Object.prototype.type = function (constructor, name) {
    ...
    Patch.assert(typeof constructor === 'function', 'type must be a constructor function');
