...
Util.inherits(internals.Date, Any);
...
internals.compare = function (type, compare) {
    ...
    return function (date) {
        ...
        Patch.assert(date, 'Invalid date format');
...
internals.Date.prototype.format = function (format) {
    ...
    Patch.assert(typeof format === 'string' || (Array.isArray(format) && format.every((f) => typeof f === 'string')), 'Invalid format.');
...
internals.Date.prototype.timestamp = function (type) {
    ...
    Patch.assert(allowed.indexOf(type) !== -1, '"type" must be one of "' + allowed.join('", "') + '"');
